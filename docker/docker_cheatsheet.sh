...
* [Do not use cache when building the image] docker build --no-cache
* [Run nginx with local volumn] docker run -itd --rm -p 8080:80 -v /root/blws/ngwww:/usr/share/nginx/html:ro nginx
* 


## [Base](https://github.com/wsargent/docker-cheat-sheet)
...
Mac OS X
Download and install Docker Toolbox. Docker For Mac is nice, but it's not quite as finished as the VirtualBox install. See the comparison.
